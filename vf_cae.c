...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame) {
    ...
    if (compute_metrics && s->window_filled) {
        ...
        // Calculate an activity score based on the weighted sum
        s->activity_score = weighted_sum;
        ...
        if (s->activity_score > high_activity_threshold && s->frame_interval > s->min_frame_interval) {
            s->frame_interval--;
            av_log(ctx, AV_LOG_INFO, "High activity detected. Decreasing frame_interval to %d.\n", s->frame_interval);
        }
        else if (s->activity_score < low_activity_threshold && s->frame_interval < s->max_frame_interval) {
            s->frame_interval++;
            av_log(ctx, AV_LOG_INFO, "Low activity detected. Increasing frame_interval to %d.\n", s->frame_interval);
        }
...
static const AVOption cae_options[] = {
    ...
    { "frame_interval", "Initial number of frames between each scene change check", OFFSET(frame_interval), AV_OPT_TYPE_INT, {.i64 = 2}, 1, 1000, FLAGS },
    ...
    { "max_frame_interval", "Maximum frame interval for adaptive processing", OFFSET(max_frame_interval), AV_OPT_TYPE_INT, {.i64 = 30}, 1, 1000, FLAGS },
    { NULL }
};
